...
- 的外れの場合 → 0〜3点（内容がどれだけ的外れかで判断）

STEP2: 的外れでない場合は以下で採点
- 内容点（0〜7点）: 模範解答の要点をどれだけ含んでいるか
  - 7点: 要点をほぼ全て含む / 5〜6点: 要点の半分以上 / 3〜4点: 一部含む / 1〜2点: わずかに関連
- 文字量ボーナス（0〜3点）: 的外れでない前提で
  - 300文字以上 → +3点 / 200文字以上 → +2点 / 100文字以上 → +1点 / 100文字未満 → +0点
- 合計 = 内容点 + 文字量ボーナス（最大10点）
【重要】文字量ボーナスは的外れでない回答にのみ適用。

必ずJSON形式のみで返答：{"scores":[{"id":"問いID","score":最終点数,"comment":"採点理由20字以内","is_off_topic":true/false}]}
...
function QuizPage({ user, session, data, onDone }) {
  ...
  const btext = cc >= 300 ? "+3点（300文字超）" : cc >= 200 ? "+2点（200文字超）" : cc >= 100 ? "+1点（100文字超）" : `あと${100 - cc}文字で+1点`;
  const bcol = cc >= 300 ? "#2E7D32" : cc >= 200 ? "#E8590C" : cc >= 100 ? "#888" : "#ccc";
...
function AdminPage({ data, cfg, onBack, updateData, updateCfg, onShowRanking }) {
  ...
  const [tf, setTf] = useState({ title: "", date: "", questions: [{ title: "", question: "", model: "", maxScore: 10 }] });
  ...
  const addTest = () => {
    ...
    const qs = tf.questions.map((q, i) => { const qid = "q" + i + "_" + id; ma[qid] = q.model; return { id: qid, title: q.title, question: q.question, maxScore: q.maxScore || 10 }; });
    ...
    setTf({ title: "", date: "", questions: [{ title: "", question: "", model: "", maxScore: 10 }] }); setShowAdd(false); setSelSId(id);
  };
