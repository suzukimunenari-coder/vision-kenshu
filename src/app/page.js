...
const DEFAULT_SESSIONS = [{
  ...
  id:"test_0415",title:"テスト③ 経営理念・お客様方針・未来会計図",date:"2026/4/15",totalScore:60,
  modelAnswers:{
    ...
    t3_q5_kando:"①4.8　②5.0　③1位　④4　⑤1　⑥2　⑦3\n※数値が合えば単位なしでも正解（例：4.8→4.8、1位→1どちらもOK）",
  ...
  questions:[
    ...
    {id:"t3_q3_omoimasu",title:"問2：「〇〇だと思います」がダメな理由",question:"お客様への回答で「〇〇だと思います」という言葉を禁じています。なぜダメなのか。またすぐに答えられない時はどうすればよいか答えてください。",maxScore:10,scoringMode:"理解型（文字量あり）"},
    {id:"t3_q4_wasure",title:"問3：宿題を忘れることが信頼問題な理由",question:"前回の打ち合わせで頼まれたことを忘れた場合、お客様はどう感じますか？また、どうすれば防げますか？",maxScore:10,scoringMode:"理解型（文字量あり）"},
    {id:"t3_q5_kando",title:"問4：利益感度分析の穴埋め",question:"下の未来会計図を見て、空欄①〜⑦を埋めてください。\n\nP（単価）感度比率=2.1%、Q（数量）感度比率=①%、V（変動費）感度比率=3.7%、F（固定費）感度比率=②%\nP感度順位=③位、Q=3位、V=2位、F=4位\n易しい順：P=④、Q=⑤、V=⑥、F=⑦",maxScore:10,scoringMode:"暗記型",imageUrl:"/images/02.jpeg",defaultAnswer:"① \n② \n③ \n④ \n⑤ \n⑥ \n⑦ "},
    {id:"t3_q6_rieki",title:"問5：理想の経常利益額",question:"下の未来会計図を見て、空欄を埋めてください。\n\nこの会社の粗利益率は（①）%\n理想の経常利益の基準は粗利益の（②）割\n粗利益額は（③）なので、理想の経常利益額は（④）",maxScore:10,scoringMode:"暗記型",imageUrl:"/images/01.jpeg",defaultAnswer:"① \n② \n③ \n④ "},
    {id:"t3_q7_mirai",title:"問6：利益とは何か・未来費用とは何か",question:"「利益とは何か」と「未来費用とは何か」をそれぞれ説明してください。",maxScore:10,scoringMode:"理解型（文字量あり）"},
  ]
}];

const COLLEGE_SESSIONS = [{
  id:"college_0413",title:"基礎テスト① 月次決算書の基本",date:"2026/4/13",totalScore:50,
  modelAnswers:{
    c1_color:"青→当月黒字・累計黒字。黄→当月黒字だが累計赤字、または当月赤字だが累計黒字。ピンク→当月赤字・累計赤字。",
    c2_pqmqfg:"PQ→売上高（単価×数量）。MQ→粗利益額（売上高－変動費）。F→固定費。G→経常利益（MQ－F）。",
    c3_explain:"P（単価）・Q（数量）・V（変動費）・F（固定費）の4つの視点で説明する。感度分析を使い、どの要素が一番利益に影響するかを数字で示しながら、どこに手を打てば利益が出るかをお客様と一緒に考える。",
    c4_salesdown:"売上三期比較グラフで傾向を確認する。粗利益率・固定費生産性・損益分岐点比率を見て、P（単価）を上げるかQ（数量）を増やすかF（固定費）を下げるかを提案する。",
    c5_cashflow:"利益は損益計算書上の数字だが、売掛金の増加・借入返済・設備投資などでお金は出ていく。キャッシュフロー計算書で実際のお金の動きを確認し、勘定合って銭足らずの状態を説明する。"
  },
  questions:[
    ...
    {id:"c3_explain",title:"問3：お客様への説明",question:"月次決算書を使って「どこに手を打てば利益が出るか」をお客様に説明する場合、どのように説明しますか？",maxScore:15,scoringMode:"理解型（文字量あり）"},
    {id:"c4_salesdown",title:"問4：売上が下がった時",question:"お客様の売上が前年比で下がっていた場合、何を見てどう対応を提案しますか？",maxScore:10,scoringMode:"理解型（文字量あり）"},
    {id:"c5_cashflow",title:"問5：利益とお金の違い",question:"「利益が出ているのにお金が残らない」という状況をどう説明しますか？",maxScore:10,scoringMode:"理解型（文字量あり）"},
  ]
}];
...
function QuizPage({ user, session, data, onDone }) {
  ...
  return <div style={S.pw} className="fade">
    ...
    <div style={S.card}>
      ...
      <textarea value={answers[q.id] || ""} onChange={e => setAnswers(p => ({ ...p, [q.id]: e.target.value }))} style={S.ta} />
