...
const DEFAULT_SESSIONS = [{
  ...
  questions:[
    ...
    {id:"q4_stage1",title:"問4：第1ステージ",question:"従業員の未来像の第1ステージで意識するべきことは何ですか？",maxScore:10,scoringMode:"理解型"},
...
function AdminPage({ data, cfg, onBack, updateData, updateCfg, onShowRanking }) {
  ...
  const [tf, setTf] = useState({ title: "", date: "", questions: [{ title: "", question: "", model: "", maxScore: 10, scoringMode: "理解型" }] });
  ...
  const addTest = () => {
    ...
    const qs = tf.questions.map((q, i) => { const qid = "q" + i + "_" + id; ma[qid] = q.model; return { id: qid, title: q.title, question: q.question, maxScore: q.maxScore || 10, scoringMode: q.scoringMode || "理解型" }; });
    ...
    setTf({ title: "", date: "", questions: [{ title: "", question: "", model: "", maxScore: 10, scoringMode: "理解型" }] }); setShowAdd(false); setSelSId(id);
  };
